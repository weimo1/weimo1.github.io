...
这就是所谓的**工厂模式**：“枪”就是“子弹”对象的工厂。 传给玩家的是子弹的工厂——枪，而不是子弹本身。 只要调用工厂的 shoot 函数，玩家可以源源不断地创建新子弹出来。 正所谓授人以鱼不如授人以渔，你的玩家不再是被动接受子弹，而是可以自己创造子弹了！
...
这里 C++ 标准保证了 unique\_ptr 的生命周期是这一整行（; 结束前），整个 player 执行期间都活着，不会提前释放 正如 `func(string().c_str())` 不会有任何问题，string 要到 func 返回后才释放呢！
...
警告：这种写法同样不适用于多 DLL 的情况！如果需要在多 DLL 环境中使用，请乖乖[分离声明和定义](https://142857.red/book/symbols/)。
...
警告：这种写法同样不适用于多 DLL 的情况！如果需要在多 DLL 环境中使用，请乖乖[分离模板的声明和定义](https://142857.red/book/symbols/)。
...
这时候就可以运用模板模式，把原来的虚函数接口改为 protected 的函数，且名字改为 do\_process。
...
标准库中的 `std::pmr::memory_resource`、`std::codecvt` 等都运用了 do\_xxx 式的模板模式封装。
...
如果调用者需要的是 shared\_ptr，怎么办？

答：unique\_ptr 可以隐式转换为 shared\_ptr。

如果调用者需要的是手动 delete 的原始指针，怎么办？

答：unique\_ptr 可以通过 release，故意造成一次内存泄漏，成为需要手动管理的原始指针。
...
1. C++（和大多数非脚本语言都）不支持运行时添加或删除基类，也就是说，如果要添加一个新角色，或是修改现有角色的逻辑，就需要重新编译一遍整个游戏的源码。
...
1. Java 和 C# 都没有多重继承。你让人家基于 C# 的 Unity 怎么活？
...
神奇的是，如果你的 Component 全部都是用 make\_shared 创建的，那就没有内存泄漏了，这得益于 shared\_ptr 会对 deleter 做类型擦除。 make\_unique 和 new 创建的就会泄漏，因为他们 delete 时是以基类指针去 delete 的，而 shared\_ptr 会在构造时就记住子类的 deleter。
...
1. 不再需要定义 Player 类及其构造函数了，只需一个普通函数创建具有 Player 所需所有组件的 GameObject 对象即可。
...
1. 允许用户根据其他组件的类型，直接获取出其他组件的指针，即可访问其成员。
...
1. 让 PlayerController 发出指定类型的消息对象，由 Movable 检查并处理。

消息类型也是多态的，初学者可以先通过 `dynamic_cast` 实现类型检查。稍后我们会介绍更专业的访问者模式。

通常来说，我们只能把子类指针转换为基类指针。

而 dynamic\_cast 可以把基类指针转换为子类指针。
...
策略模式最精妙的 C++ 实现就是虚函数替换。回顾文中的 `reduce` 函数：

```cpp
int reduce(vector<int> v, Reducer* reducer) {
    int res = reducer->init();        // 虚函数调用①
    for (int i = 0; i < v.size(); i++) {
        res = reducer->add(res, v[i]); // 虚函数调用②
    }
    return res;
}
```

`reducer->add()` 表面是一行代码，背后是两次指针解引用：`reducer->vtable[1]()`。新增策略（如 `MinReducer`）零修改 `reduce` 函数——这是开闭原则的经典体现。

#### 观察者模式与发布-订阅的 type_index 路由

```cpp
// 事件总线的核心：type_index 路由表
template <class EventType>
void send(EventType* msg) {
    auto it = subscribers.find(std::type_index(typeid(EventType)));
    if (it != subscribers.end()) {
        for (auto* c : it->second) {
            c->handleMessage(msg);  // 只通知订阅者
        }
    }
}
```

`type_index` 作为路由键，将 O(n) 的广播优化为 O(1) 的定向分发。`dynamic_cast` 检查在发布-订阅中仅需一次 `typeid` 比对，性能远优于观察者的逐个 cast。

#### 组件模式中 getComponent 的 typeid 实现
...
T* getComponent() {
    ...
    if (it != components.end()) {
        return static_cast<T*>(it->second);  // 已知类型安全，无需 dynamic_cast
    }
    return nullptr;
}
```

组件注册时用 `typeid(*component)` 做键，查找时用 `typeid(T)` 精确匹配，O(1) 查找 + `static_cast`（零开销），比遍历 `dynamic_cast` 高效得多。

### 5.2 实践经验

1. **策略模式 + 工厂模式 = 可插拔算法库**：策略定义"做什么"、工厂定义"谁来做"，组合到一起可实现运行时切换算法
2. **组件模式是 ECS 的基础**：Unity 等引擎的 GameObject + Component 就是组件模式，扩展新功能只需加新组件
3. **CRTP 替代虚函数有陷阱**：CRTP 在编译期绑定，无法实现运行时多态（不能放进 `vector<Base*>`），仅适用于类型已知的场景
4. **观察者模式慎用**：组件间广播消息虽解耦但难调试——消息流不透明，容易形成"消息风暴"
5. **享元模式不只收游戏**：字符串 intern、线程池复用、protobuf arena 都是享元思想
6. **模板模式 vs 策略模式的选择口诀**：一个变数用模板模式，多个独立变数用策略模式

## 六、面试准备

### 6.1 面试 Q&A

**Q1: 策略模式、模板方法模式、状态模式三者如何区分？**

| 模式 | 变化点 | 谁决定行为 | C++ 实现 |
|------|--------|-----------|----------|
| 策略 | 算法可替换 | 外部调用者传入 | 接口 + 多态注入 |
| 模板方法 | 步骤可重写 | 基类控制骨架 | 虚函数 + NVI |
| 状态 | 行为随状态变 | 内部状态对象 | 状态对象 + 委托 |

**Q2: 组件模式相比多重继承有什么优势？**

① 运行时组合：可在运行时添加/移除组件，多重继承是编译期绑定的；② 避免菱形继承：组件模式是一对多组合关系；③ 脚本友好：Lua/蓝图可动态为对象挂载组件。

**Q3: 享元模式和单例模式的区别？**

单例保证**全局唯一实例**（如配置管理器），享元是**共享多个同类对象中的不变部分**（如字体渲染中共享字形数据）。单例的对象只有一个，享元的**内在状态**只存一份但**外在状态**各有不同。

**Q4: 发布-订阅和观察者模式的核心差异？**

观察者：被观察者直接维护观察者列表，两者强耦合。发布-订阅：通过事件总线/消息队列解耦，发布者不知道订阅者是谁。前者是同步的（直接调用），后者可以是异步的（消息队列）。

**Q5: 原型模式的 clone 为什么不能用拷贝构造函数替代？**

拷贝构造函数在编译期需要知道具体类型——`Ball newball = *ball` 会发生 object slicing。`clone()` 是虚函数，运行时按实际类型创建副本，保持多态性。

**Q6: 工厂模式与 Builder 模式的区别？**

工厂模式：一次调用产出对象（`gun->shoot()`）。Builder：分步构建复杂对象（`builder.setA().setB().build()`）。工厂关注**创建什么**、Builder 关注**如何创建**。

**Q7: 适配器模式和代理模式的区别？**

适配器改变接口（A 接口转成 B 接口），代理不改变接口（只是转发）。适配器 = 翻译器（220V→110V），代理 = 门卫（检查权限后放行）。

**Q8: MVC 模式在游戏开发中如何落地？**

以 Unity 为例：Model = ScriptableObject 数据容器，View = UI Canvas + Image/Text 组件，Controller = MonoBehaviour 脚本处理输入并更新 Model。Web 中 Model=数据库、View=HTML、Controller=后端路由。

**Q9: 装饰器模式 和 代理模式 的区别？**

装饰器**增强**功能（给咖啡加糖），代理**控制**访问（权限检查）。装饰器可以多层嵌套（加糖又加奶），代理通常单层。C++ 中 `std::fstream` 和 `std::stringstream` 的 `std::ios_base` 继承链就是装饰器。

**Q10: 责任链模式在游戏中的典型应用？**

碰撞检测：伤害事件沿 chain（护盾 → 护甲 → 血量）传递，每个节点决定是否处理或传给下一个。C++ 实现：
```cpp
struct Handler { Handler* next; virtual void handle(Damage& d) {
    if (canHandle(d)) apply(d); else if (next) next->handle(d);
}};
```

### 6.2 常见陷阱与面试反问

1. **陷阱**：策略模式滥用 → 每个小行为都抽一个类 → 类爆炸。**修复**：优先用 lambda/std::function 替代简单策略。

2. **陷阱**：状态模式的状态对象不断 new/delete → 内存抖动。**修复**：用对象池预分配状态对象，或使用静态单例状态。

3. **陷阱**：观察者模式中观察者的生命周期短于被观察者 → 被观察者持有悬空指针。**修复**：被观察者存 `weak_ptr`，通知前 `lock()` 检查。

4. **反问**：「你如何决定一个场景用哪种设计模式？」希望听到：先分析变化点和稳定点，稳定点封装为接口、变化点实现为策略/状态/组件。不要为了用模式而用模式。

5. **反问**：「设计模式在 C++ 中的性能开销如何？」希望听到：虚函数调用（~1-2ns 间接跳转）、动态分配（策略/状态对象）、typeid 查找。通常可忽略，热路径可考虑 CRTP/模板替代。
...
| 享元模式 | 共享不变部分，节省内存 |
| 组件模式 | 运行时拼装功能，告别多重继承 |
| 观察者模式 | 一对多通知，被观察者主动推送 |
| 发布-订阅 | 事件总线解耦，按 type 路由 |
| 状态模式 | 状态=类，行为随状态切换 |
| 原型模式 | clone() 深拷贝多态对象 |
| CRTP | 编译期多态，替代虚函数 |
| 访问者模式 | 双向 dispatch，分离操作与结构
